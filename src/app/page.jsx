  ...
  return (
    <main className='flex flex-col md:flex-row gap-10 md:h-screen pt-20 pb-10 transition-colors duration-300 ease-out'>
      <div className='md:w-1/2 lg:w-2/3 flex flex-col justify-between gap-10'>
        <div className='h-full min-h-[20em] lg:h-2/3 flex justify-center items-center bg-slate-100 dark:bg-zinc-800 shadow-md p-6 rounded-lg -z-10 lg:z-0'>
          <div
          ...
          <div className=''>
            ...
            <div className='mt-2 flex items-center justify-between gap-6 bg-slate-50 dark:bg-zinc-900 p-4 rounded-lg shadow-md'>
              <input
                ...
                readOnly
                value={
                  `${rotateValue > 0 ? `rotate-[${rotateValue}deg] ` : ''}` +
                    `${scaleValue != 1 ? `scale-[${scaleValue}] ` : ''}` +
                    `${skewValue > 0 ? `skew-x-[${skewValue}deg] ` : ''}` +
                    `${
                      translateXValue != 0
                    ...
                    `${
                      translateYValue != 0
                        ...
                        : ''
                    }`.trim() || 'No transformation applied'
                }
                ...
                value={
                  ...
                  translateYValue == 0
                    ...
                    : `transform: ${
                        ...
                        skewValue > 0 ? `skew(${skewValue}deg) ` : ''
                      }${
                        translateXValue != 0
      ...
      <div className='md:w-1/2 lg:w-1/3 bg-slate-100 dark:bg-zinc-800 shadow-md p-6 rounded-lg'>
        ...
        <div className='mt-6'>
          <SliderInput
            state={rotateValue}
            handler={value => handleSlider(value, 'rotate')}
            title={'Rotate'}
            defaultValue={0}
            value={[rotateValue]}
            max={360}
          ...
          <SliderInput
            state={scaleValue}
            handler={value => handleSlider(value, 'scale')}
            title={'Scale'}
            value={scaleValue}
            defaultValue={1}
            max={2}
          ...
          <SliderInput
            state={skewValue}
            handler={value => handleSlider(value, 'skew')}
            title={'Skew'}
            value={skewValue}
            defaultValue={0}
            max={180}
          ...
          />
          <SliderInput
            state={translateXValue}
            handler={value => handleSlider(value, 'translateX')}
            title={'Translate X'}
          ...
          />
          <SliderInput
            state={translateYValue}
            handler={value => handleSlider(value, 'translateY')}
            title={'Translate Y'}
